...
JobZy Backend is the server-side of a full-stack job portal built using the MERN stack. It handles authentication, user management, job operations, and application workflows for both students and recruiters. Built with Node.js, Express, MongoDB, and Mongoose, it includes JWT-based authentication and secure password hashing with bcrypt. The backend supports features like job posting, applying, applicant tracking, and status updates, with a scalable architecture organized into controllers, routes, models, and middleware. Designed for performance and future enhancements like file uploads (Multer, Cloudinary). 🚀
